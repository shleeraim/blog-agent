...
## 응답 형식 (반드시 이 순서·형식 그대로 — 절대 어기지 말 것)
JSON 안에 블로그 본문 전체를 넣지 마라. 따옴표나 줄바꿈을 한 글자라도 잘못 escape하면
전체 응답이 깨진다. 대신 아래처럼 "메타데이터 JSON" 한 덩어리와 "본문 마크다운"을
구분자로 나눠서 출력하라:

1. 첫 줄부터: 아래 메타데이터 필드만 담은 JSON 한 덩어리 (content, word_count 넣지 말 것)
2. JSON이 끝나면 바로 다음 줄에 구분자를 단독으로: ===DRAFT_BODY===
3. 그 아래: 블로그 본문 전체를 마크다운으로, escape 없이 그대로 작성

마크다운 코드블록(\`\`\`)으로 감싸지 말고, JSON 앞뒤로 다른 설명 텍스트도 출력하지 마라.

## 메타데이터 JSON 스키마 (구분자 이전)
{
...
## 출력 예시 (형식만 참고, 내용은 실제 글에 맞게 작성)
{"meta_title":"...","meta_desc":"...","category":"...","tags":["...","..."],"seo_tips":["...","..."]}
===DRAFT_BODY===
## 첫 번째 소제목
본문 내용...`;
}
